...
def ShrinkTop(X, shrinkArg):
    ...
    return block_reduce(X.reshape(X.shape[0], X.shape[1], X.shape[2], -1), (1, 2, 2, 1), np.average)
...
SaabArgs = [{'num_AC_kernels':-1, 'needBias':False, 'useDC':True, 'batch':None, 'cw':False}, 
            {'num_AC_kernels':-1, 'needBias':True, 'useDC':True, 'batch':None, 'cw':False},
...
lag_rep = [
    ...
    LAG(encode='distance', num_clusters=[5] * 10, alpha=10, learner=LLSR(onehot=False)),
    LAG(encode='distance', num_clusters=[4] * 10, alpha=5, learner=LLSR(onehot=False)),
    LAG(encode='distance', num_clusters=[4] * 10, alpha=5, learner=LLSR(onehot=False)),
    ]
