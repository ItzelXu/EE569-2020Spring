...
f = open('pixelHop2_AVG_single.pkl', 'wb')
...
augment_size = int((1/5) * train_later_size)
...
class Cal_ce:
    def __init__(self, output_train_sub):
        self.ce = Cross_Entropy(num_class=10, num_bin=10)
...
lag_rep = [
    LAG(encode='distance', num_clusters=[8] * 10, alpha=10, learner=LLSR(onehot=False)),
...
svmclf = svm.SVC(C =2, kernel='rbf', gamma=11) # 50000
...
f = open('x_test_trans_single.pkl', 'wb')
...
# #         x_train_trans = np.concatenate((x_train_trans, lag_rep[i].transform(output_i_train)), axis=1)
#
f = open('x_train_trans_single.pkl', 'wb')
pickle.dump(x_train_trans, f)
f.close()
#
# score_tr = rfclf.score(x_train_trans, np.squeeze(y_train_later_ori))
# print('Train score =', score_tr)
